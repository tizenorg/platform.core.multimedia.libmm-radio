...
static void __mmradio_sound_focus_cb(int id, mm_sound_focus_type_e focus_type, mm_sound_focus_state_e focus_state, const char *reason_for_change, const char *additional_info, void *user_data);
...
int _mmradio_create_radio(mm_radio_t *radio)
{
	...
	ret = mm_sound_add_device_connected_callback(MM_SOUND_DEVICE_STATE_ACTIVATED_FLAG, (mm_sound_device_connected_cb)__mmradio_device_connected_cb, (void *)radio, &radio->subs_id);
	if (ret) {
...
int _mmradio_realize_pipeline(mm_radio_t *radio)
{
	...
	gst_bin_add_many(GST_BIN(radio->pGstreamer_s->pipeline), radio->pGstreamer_s->audiosrc, radio->pGstreamer_s->queue2, radio->pGstreamer_s->audiosink, NULL);
	if (!gst_element_link_many(radio->pGstreamer_s->audiosrc, radio->pGstreamer_s->queue2, radio->pGstreamer_s->audiosink, NULL)) {
...
static int __mmradio_check_state(mm_radio_t *radio, MMRadioCommand command)
{
	...
	case MMRADIO_COMMAND_REALIZE:
		{
			if (radio_state == MM_RADIO_STATE_READY || radio_state == MM_RADIO_STATE_PLAYING || radio_state == MM_RADIO_STATE_SCANNING)
				goto NO_OP;
